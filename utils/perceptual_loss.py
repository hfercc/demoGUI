...
def l_cs(img1, img2, window, channel):
    padd = 0
...
def ssim(img1, img2, window_size=64, sigma=5, size_average=True, full=False):
...
class SSIM(torch.nn.Module):
    def __init__(self, window_size=64, size_average=True):
    ...
    def forward(self, img1, img2):
        ...
        return 1 - _ssim(img1, img2, self.window, channel, self.size_average)
...
def msssim(img1, img2, window_size=64, sigma=[0.5, 1., 2., 4., 8.], size_average=True):
    if img1.size() != img2.size():
        raise RuntimeError('Input images must have the same shape (%s vs. %s).' %
                           (img1.size(), img2.size()))
    if len(img1.size()) != 4:
...
class MSSSIM(torch.nn.Module):
    def __init__(self, window_size=64, sigmas=[0.5, 1., 2., 4., 8.], size_average=True, channel=1):
...
class MSSSIM_L1(torch.nn.Module):
    def __init__(self, alpha = 0.84, window_size=64, sigmas=[0.5, 1., 2., 4., 8.], size_average=True, channel=1):
