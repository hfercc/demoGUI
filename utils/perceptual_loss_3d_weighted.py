'''
pytorch version: https://github.com/jorge-pessoa/pytorch-msssim/blob/master/pytorch_msssim/__init__.py
origin MSSSIM in caffe: https://github.com/NVlabs/PL4NN/blob/master/src/loss.py
tensorflow: https://github.com/tensorflow/models/blob/master/research/compression/image_encoder/msssim.py
...
def create_window(window_size, channel, sigma=5):
    ...
    _3D_window = [_1D_window.mm(_2D_window[i:i+1, :]).unsqueeze(0) for i in range(window_size)]
    _3D_window = torch.cat(_3D_window, 0).float()
    _3D_window /= _3D_window.sum()
    _3D_window = _3D_window.unsqueeze(0).unsqueeze(0) # add two more dimensions
...
class SSIM(torch.nn.Module):
    def __init__(self, window_size=8, channel=1, sigma=5, weights=None, size_average=True):
        ...
        if self.weights is not None:
            self.weights = F.conv3d(self.weights, self.window, padding=0, groups=self.channel)
